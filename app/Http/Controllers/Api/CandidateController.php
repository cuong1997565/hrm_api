...
class CandidateController extends ApiController
{
    ...
    */
    ...
    public function import(Request $request)
    {
        try {
            if ($request->hasFile('file')) {
                ...
                $this->candidate->import($path);
                return $this->successResponse(['data' => 'Import success'], false);
            } else {
                return $this->errorResponse([
                    'errors' => 'Vui lòng chọn file có dung lượng nhỏ hơn'
                ]);
            }
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return $this->notFoundResponse();
    ...
    {
        try {
            $this->candidate->export($type);
            return $this->successResponse(['data' => 'Export success'], false);
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return $this->notFoundResponse();
        } catch (\Exception $e) {
            throw $e;
    ...
    {
        try {
            $this->candidate->download($type);
            return $this->successResponse(['data' => 'Download success'], false);
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return $this->notFoundResponse();
        } catch (\Exception $e) {
            throw $e;
